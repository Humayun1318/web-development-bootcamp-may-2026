...
export const authenticate = (...authRoles: string[]) => async (req: Request, _res: Response, next: NextFunction) => {
    ...
    try {
        ...
        if (authRoles.length > 0 && !authRoles.includes(verifiedToken.role)) {
            throw new AppError(HTTP_STATUS.FORBIDDEN, 'You do not have permission to access this resource');
        }
